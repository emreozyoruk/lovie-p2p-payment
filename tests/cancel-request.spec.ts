...
test.describe('Cancel Request', () => {
  test('sender can cancel their request', async ({ page }) => {
    await login(page);
    await page.goto('/requests/new');
    await page.getByLabel('Recipient Email').fill('cancel-e2e@example.com');
    await page.getByLabel('Amount (USD)').fill('35');
    await page.getByLabel('Note').fill('Cancel E2E');
